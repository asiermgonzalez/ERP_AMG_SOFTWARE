...
export class EditRolesComponent {
  @Output() RoleE: EventEmitter<any> = new EventEmitter(); // Emite datos hacia el padre
  ...
  update() {
    ...
    // Se llama al servicio para editar un nuevo rol
    this.rolesService.updateRole(this.ROLE_SELECTED.id, data).subscribe((resp: any) => {
      ...
      if (resp.message == 403) {
        ...
        this.toast.success('Rol creado', 'El rol se ha editado correctamente');
        this.RoleE.emit(resp.role); // Se emite el rol creado hacia list-roles.component.ts
